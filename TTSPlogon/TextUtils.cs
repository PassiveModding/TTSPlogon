...
namespace TTSPlogon
{
    public static partial class TextUtils
    {
        ...
        public static string? CleanText(string? text)
        {
            ...
            text = StripAngleBracketedText(text);
            text = ReplaceSsmlTokens(text);
            text = NormalizePunctuation(text);
            text = RemoveStutters(text);
            text = text.Trim();
            if (text.Length == 0) return null;
            if (!IsSpeakable(text)) return null;
            return text;
        }
        
        public static unsafe (string speaker, string text) ReadTalkAddon(AddonTalk* talkAddon)
        {
            return new(
